...
impl Content {
    pub fn get_solution(&self) -> Option<Solution> {
        match self {
            Content::SQL { solution, .. } => Some(Solution::SQL(solution.clone())),
            Content::MC { solution, .. } => Some(Solution::MultipleChoice(solution.clone())),
            Content::Plaintext { solution, .. } => Some(Solution::Text(solution.clone())),
            _ => None,
        }
    }
}
